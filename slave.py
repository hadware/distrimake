...
class Slave:
    def __init__(self, name):
        self.name = name
        self.dispatcher = Pyro4.core.Proxy("PYRONAME:%s" % dis.DISPATCHER_NAME)
    ...
    def run(self):
        while True:
            ...
            except dis.NoJobAvailableYet:
                sleep(2)
            except dis.AllJobsCompleted:
    ...
    def _execute_job(self, job):
        ...
        try:
            logging.debug("Executing job : %" % '; '.join(job.rule.commands))
            job.execute()
